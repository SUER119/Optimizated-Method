...
#    message(STATUS "Fetching from URL: https://anaconda.org/intel/${name}/${MKL_VERSION}/download/${MKL_PLATFORM}/${name}-${MKL_PLATFORM}-file")

#    FetchContent_Declare(
#       ${name}
#       URL https://anaconda.org/intel/${name}/${MKL_VERSION}/download/${MKL_PLATFORM}/${name}-${MKL_PLATFORM}-file
#       # 如果不想使用 MD5 校验，可以暂时注释掉以下一行
...
set(MKL_INCLUDE_DIR /opt/intel/oneapi/mkl/2025.3/include)

# 如果 CMake 需要额外的路径，添加到 include_directories
...
function(mkl_add_imported_library name)
    ...
    # 手动设置库路径
    set(MKL_LIB_HINTS "/opt/intel/oneapi/mkl/2025.3/lib")
   

    # 查找 MKL 核心库
    string(TOUPPER mkl_${name}_library LIBVAR)
    find_library(${LIBVAR}
        NAMES mkl_core${MKL_LIB_SUFFIX}  # 确保库名称正确
        HINTS ${MKL_LIB_HINTS}
        NO_DEFAULT_PATH
    )
